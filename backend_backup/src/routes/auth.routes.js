...
router.get('/steam/return', 
  passport.authenticate('steam', { failureRedirect: '/api/auth/login-failed' }),
  asyncHandler(async (req, res) => {
    // req.user aqui contém o que o done() do passport.js retornou
    ...
    if (!user) {
      // Se não existe, criamos um utilizador "Steam" temporário ou pedimos para vincular
      // Por agora, vamos criar um utilizador básico se não existir
      user = await User.create({
        name: req.user.profile?.displayName || 'Steam User',
        email: `${steamId}@steam.wealthsphere.app`, // Email fictício pois Steam não dá email
        passwordHash: 'steam-login-no-password',
        'externalApis.steam.steamId': steamId
      });
    }
    ...
    const targetUrl = `${env.frontendUrl}/auth?token=`;

    res.redirect(`${targetUrl}${accessToken}&refresh=${refreshToken}`);
  })
);
